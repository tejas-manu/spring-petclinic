pipeline {
  ...
  environment {
    ...
    ECR_REPOSITORY_URI = '318488421234.dkr.ecr.us-east-1.amazonaws.com/spring-boot/petclinic'
      ...
      steps {
        ...
        withSonarQubeEnv('MySonarServer') {
          sh "mvn sonar:sonar \
              ...
              -Dsonar.host.url=http://18.208.229.208:9000/"
          }
      }
    }
    ...
    stage('Run Application for Scan') {
      steps {
        script {
          ...
          APP_CONTAINER_ID = sh(returnStdout: true, script: "docker run -d -p 8080:8080 ${DOCKER_IMAGE}").trim()
